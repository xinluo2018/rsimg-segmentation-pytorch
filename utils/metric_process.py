...
def smooth(y, window=31, num_sam = None):
    '''
    des: smoothing the metric along the x-axis
    args:
      y: values of the metric
      window: the interpolation window/size
      num_sam: number of the output values.
    '''
...
def csv_merge(csv_files, i_csv = None, i_row=None, sam=None):
    ...
    usage: metric_merge = csv_merge(paths_metric, i_csv='i_test', i_row='epoch', sam=5)
    args:
